...
Polaris.prototype.boot = function () {
    ...
    process.on('uncaughtException', this._handleShutdown.bind(this));
...
/**
 * Called on uncaught exceptions and shutdowns
 * @param  {Error=} err
 */
Polaris.prototype._handleShutdown = function (err) {
    ...
    // If it's the leader, be sure to emit progress before closing.
    if (!this.isLeader()) {
        this._heartbeat().finally(function () {
            console.log('exiting');
...
Polaris.prototype._heartbeat = function () {
    ...
    // current job statuses.
        ...
        return this._raft.writeTo(this._raft.LEADER, 'job:report', updates);
    }
};
